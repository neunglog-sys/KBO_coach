...
// 배트 스윙 감지용 — 홈런/안타/파울/배트 등 '치다/휘두르다' 맥락이면 배트 스윙(swing) 1회 재생.
// (안타깝다 오탐 방지: 안타 뒤 '깝' 제외)
const SWING_RE = /(홈런|안타(?!깝)|파울|배트|방망이|스윙|휘둘|휘두르|타격|장타|루타|쳤|hit|swing|home\s?run)/i;
...
type MotionKind = "greet" | "run" | "throw" | "swing";
function buildMotionCues(text: string): Array<{ at: number; kind: MotionKind }> {
  ...
  const w = text.match(SWING_RE);
  if (w && w.index != null) cues.push({ at: w.index + w[0].length, kind: "swing" });
  return cues;
}
...
export function MainViewV2({
  ...
  // 값이 증가할 때마다 캐릭터가 배트 스윙(swing) 모션을 1회 재생.
  ...
  const pendingSwingRef = useRef(false);
    ...
    const fireCuesUpTo = (spokenChars: number) => {
      for (const c of cues) {
        if (!fired.has(c.kind) && spokenChars >= c.at) {
          ...
          else if (c.kind === "swing") setSwingSignal((n) => n + 1);
    ...
    return new Promise((resolve) => {
      ...
      audio.onplay = () => {
        ...
        for (const c of cues) {
          ...
          else if (c.kind === "swing") pendingSwingRef.current = false;
  ...
  async function submitQuestion(raw: string) {
    ...
    pendingSwingRef.current = SWING_RE.test(answer) || SWING_RE.test(question);
  ...
  useEffect(() => {
    if (isSpeaking && !prevSpeakingRef.current) {
      ...
      if (pendingSwingRef.current) {
        pendingSwingRef.current = false;
        setSwingSignal((n) => n + 1);
      }
    }
